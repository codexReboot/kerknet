...
async function startServer() {
  ...
  app.get("/liturgie", requireAuth, (req, res) =>
    res.render("welcome", { title: "Home", message: "Hello, world!" })
  );
  app.get("/liturgie/eerste", requireAuth, (req, res) =>
    res.render("eerste", { title: "Eerste Diens", message: "Hello, world!" })
  );

  // -------------------- Helpers --------------------
  const BOOK_NAMES_PATTERN = [
    "Genesis","Exodus","Levitikus","Numeri","Deuteronomium","Josua","Rigters","Rut",
    "1\\s*Samuel","2\\s*Samuel","1\\s*Konings","2\\s*Konings","1\\s*Kronieke","2\\s*Kronieke",
    "Esra","Nehemia","Ester","Job","Psalms","Spreuke","Prediker","Hooglied","Jesaja","Jeremia",
    "Klaagliedere","Esegiel","Daniel","Hosea","Joel","Amos","Obadja","Jona","Miga","Nahum","Habakuk",
    "Sefanja","Haggai","Sagaria","Maleagi","Mattheus","Markus","Lukas","Johannes","Handelinge",
    "Romeine","1\\s*Korinthiers","2\\s*Korinthiers","Galasiers","Efesiers","Filippense","Kolossense",
    "1\\s*Thessalonicense","2\\s*Thessalonicense","1\\s*Timotheus","2\\s*Timotheus","Titus","Filemon",
    "Hebreers","Jakobus","1\\s*Petrus","2\\s*Petrus","1\\s*Johannes","2\\s*Johannes","3\\s*Johannes",
    "Judas","Openbaring"
  ].join("|");
  ...
  app.post("/preview-slide", (req, res) => {
    try {
      let {
        datum, diensVolgorde, prediker, skrifgedeelte, teksverse,
        skriflesingText, teksverseText, tema, kollektesErediens,
        kollektesDeure, voorsangType, voorsangNumber, voorsangVerses,
        lofPsalmSeengroetType, lofPsalmSeengroetNumber, lofPsalmSeengroetVerses,
        psalmNaWetType, psalmNaWetNumber, psalmNaWetVerses,
        psalmVoorWoordType, psalmVoorWoordNumber, psalmVoorWoordVerses,
        psalmSlotType, psalmSlotNumber, psalmSlotVerses
      } = req.body;
      ...
      const commandmentSlides = [
        `<sup>1</sup> TOE het God al hierdie woorde gespreek ...`,
        `<sup>4</sup> Jy mag vir jou geen gesnede beeld ...`,
        `<sup>9</sup> Ses dae moet jy arbei ...`,
        `<sup>13</sup> Jy mag nie doodslaan nie ...`,
        `<sup>17</sup> Jy mag nie jou naaste se huis begeer nie ...`,
      ];
      ...
      res.render("slides", {
        datum, diensVolgorde, prediker, skrifgedeelte, teksverse,
        scriptureSlides, teksverseSlides, creedSlides, commandmentSlides,
        songs, tema, kollektesErediens, kollektesDeure,
        preScriptureTransitions, postScriptureTransitions,
      });
    } catch (err) {
